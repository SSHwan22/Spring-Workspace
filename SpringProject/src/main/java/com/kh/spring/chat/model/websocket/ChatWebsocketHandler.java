...
public class ChatWebsocketHandler extends TextWebSocketHandler{
	 ...
	 * -> WebSocketHandler인터페이스를 구현하는 클래스(TextWebSocketHandler)을 이용해서 웹소켓 기능을 구현할 예정
	 * 
	 * * 웹소켓핸들러 주요 메서드 *
	 * 
	...
	// -> 멀티쓰레드환경에서 하나의 컬렉션 요소에 여러쓰레드가 동시에 접근하면 충돌이 발생할 수 있으므로 동기화를 진행.
	...
	public void afterConnectionEstablished(WebSocketSession session) {
		...
		sessions.add(session); // 전달받은 session를 set에 추가
	...
	public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
		...
		// 웹소켓 연겨이 종료되는 경우, sessions안에 저장되어있던 클라이언트의 session정보를 삭제
	}
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		if(result>0) {
			...
			for(WebSocketSession s : sessions) {
				...
				// 반복을 진행 중인 WebSocketSession안에 담겨있는 방 번호 == 메세지 안에 담겨있는 일치하는 경우 메세지 뿌리기
				...
				if(chatMessage.getChatRoomNo() == chatRoomNo) {
					...
					// s.sendMessage( new TestMessage( JSON객체 ))
