...
@Component	// 런타임시 필요한 위치에 코드를 끼워넣을 수 있드록 bean으로 등록시켜주기
@Aspect		// 공통관심사가 작성된 클래스임을 명시
			// *공통관심사? 특정 흐름 사이에 끼여서 수행할 코드
			// Aspect어노테이션이 붙은 클래스에는 실행할 코드(advice)와 pointCut이 정의되어 있어야한다.
			// @PointCut(advice가 끼여들어서 수행될 클래스, 메서드 위치등을 정의)이 작성되어 있어야 한다.
public class Test {

	private Logger logger = LoggerFactory.getLogger(Test.class);
	
	// joinPoint : 클라이언트가 호출하는 모든 비즈니스 메서드. advice가 적용될수 있는 예비 후보
	 ...
	 * PointCut 작성 방법 
	 * 
	 * @PointCut("execution([접근제한자] 반환형 패키지+클래스명.메서드([매개변수]))")
	 * PointCut 표현식
	 * * : 모든 | 아무값
	 ...
	 * 
	 * com.kh.spring.board 패키지 아래에 있는 Impl로 끝나는 클래스의 모든 메서드에(매개변수와 상관없이) 포인트컷 지정
	 */
